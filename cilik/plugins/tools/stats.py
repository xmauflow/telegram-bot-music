...
from cilik.misc import SUDOERS, pymongodb
from cilik.plugins import ALL_MODULES
from cilik.utils.database import (get_global_tops,
                                    get_particulars, get_queries,
                                    get_served_chats,
                                    get_served_users, get_sudoers,
                                    get_top_chats, get_topp_users)
from cilik.utils.decorators.language import language, languageCB
from cilik.utils.inline.stats import (back_stats_buttons,
                                        back_stats_markup,
                                        get_stats_markup,
...
@cilik.on_callback_query(filters.regex("GetStatsNow") & ~BANNED_USERS)
@languageCB
async def top_users_ten(client, CallbackQuery: CallbackQuery, _):
    ...
    if what in ["Users", "Chats"]:
        for items, count in list_arranged.items():
            ...
            try:
                extract = (
                    (await cilik.get_users(items)).first_name
                    if what == "Users"
                    else (await cilik.get_chat(items)).title
                )
                if extract is None:
                    continue
...
@cilik.on_callback_query(filters.regex("TopOverall") & ~BANNED_USERS)
@languageCB
...
@cilik.on_callback_query(filters.regex("bot_stats_sudo"))
@languageCB
async def overall_stats(client, CallbackQuery, _):
