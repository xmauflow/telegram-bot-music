...
from cilik.utils.database import (add_banned_user,
                                    get_banned_count,
                                    get_banned_users,
                                    get_served_chats,
                                    is_banned_user,
...
@cilik.on_message(filters.command(GBAN_COMMAND) & SUDOERS)
@language
async def gbanuser(client, message: Message, _):
    if not message.reply_to_message:
        ...
        user = await cilik.get_users(user)
        user_id = user.id
    ...
    for chat_id in served_chats:
        try:
            await cilik.ban_chat_member(chat_id, user_id)
            number_of_chats += 1
...
@cilik.on_message(filters.command(UNGBAN_COMMAND) & SUDOERS)
@language
async def gungabn(client, message: Message, _):
    ...
    for chat_id in served_chats:
        try:
            await cilik.unban_chat_member(chat_id, user_id)
            number_of_chats += 1
        except FloodWait as e:
            await asyncio.sleep(int(e.x))
        except Exception:
...
@cilik.on_message(filters.command(GBANNED_COMMAND) & SUDOERS)
@language
async def gbanned_list(client, message: Message, _):
    ...
    for user_id in users:
        count += 1
        try:
            user = await cilik.get_users(user_id)
            user = (
