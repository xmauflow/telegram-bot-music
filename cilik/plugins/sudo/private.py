...
from cilik.utils.database import (add_private_chat,
...
@cilik.on_message(filters.command(AUTHORIZE_COMMAND) & SUDOERS)
@language
...
@cilik.on_message(filters.command(UNAUTHORIZE_COMMAND) & SUDOERS)
@language
...
@cilik.on_message(filters.command(AUTHORIZED_COMMAND) & SUDOERS)
@language
async def authorized(client, message: Message, _):
    ...
    for served_chat in served_chats:
        try:
            title = (await cilik.get_chat(served_chat)).title
            count += 1
