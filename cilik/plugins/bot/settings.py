...
from cilik.utils.database import (add_nonadmin_chat,
...
async def settings_back_markup(
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        try:
            await cilik.resolve_peer(OWNER_ID[0])
            OWNER = OWNER_ID[0]
        except:
            OWNER = None
        buttons = private_panel(_, cilik.username, OWNER)
        return await CallbackQuery.edit_message_text(
