...
from mausic.utils.database import (add_nonadmin_chat,
...
async def settings_back_markup(
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        try:
            await mausic.resolve_peer(OWNER_ID[0])
            OWNER = OWNER_ID[0]
        except:
            OWNER = None
        buttons = private_panel(_, mausic.username, OWNER)
        return await CallbackQuery.edit_message_text(
