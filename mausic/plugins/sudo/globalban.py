...
from mausic.utils import get_readable_time
from mausic.utils.database import (add_banned_user,
                                    get_banned_count,
                                    get_banned_users,
                                    get_served_chats,
                                    is_banned_user,
...
@mausic.on_message(filters.command(GBAN_COMMAND) & SUDOERS)
@language
async def gbanuser(client, message: Message, _):
    ...
    for chat_id in served_chats:
        try:
            await mausic.ban_chat_member(chat_id, user_id)
            number_of_chats += 1
...
@mausic.on_message(filters.command(UNGBAN_COMMAND) & SUDOERS)
@language
async def gungabn(client, message: Message, _):
    ...
    for chat_id in served_chats:
        try:
            await mausic.unban_chat_member(chat_id, user_id)
            number_of_chats += 1
        except FloodWait as e:
            await asyncio.sleep(int(e.x))
        except Exception:
...
@mausic.on_message(filters.command(GBANNED_COMMAND) & SUDOERS)
@language
