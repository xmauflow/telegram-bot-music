...
from mausic.utils.database import (blacklist_chat,
                                    blacklisted_chats,
                                    whitelist_chat)
...
@mausic.on_message(filters.command(BLACKLISTCHAT_COMMAND) & SUDOERS)
@language
async def blacklist_chat_func(client, message: Message, _):
    ...
    try:
        await mausic.leave_chat(chat_id)
    except:
        pass


@mausic.on_message(filters.command(WHITELISTCHAT_COMMAND) & SUDOERS)
@language
...
async def all_chats(client, message: Message, _):
    ...
    for count, chat_id in enumerate(await blacklisted_chats(), 1):
        try:
            title = (await mausic.get_chat(chat_id)).title
        except Exception:
            title = "Private"
        j = 1
