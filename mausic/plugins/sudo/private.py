...
from mausic.utils.database import (add_private_chat,
...
@mausic.on_message(filters.command(AUTHORIZE_COMMAND) & SUDOERS)
@language
...
@mausic.on_message(filters.command(UNAUTHORIZE_COMMAND) & SUDOERS)
@language
...
@mausic.on_message(filters.command(AUTHORIZED_COMMAND) & SUDOERS)
@language
async def authorized(client, message: Message, _):
    ...
    for served_chat in served_chats:
        try:
            title = (await mausic.get_chat(served_chat)).title
            count += 1
