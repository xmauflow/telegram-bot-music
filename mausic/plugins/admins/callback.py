...
from mausic.misc import SUDOERS, db
from mausic.utils.database import (is_active_chat,
...
from mausic.utils.formatters import seconds_to_min
from mausic.utils.inline.play import (panel_markup_1,
                                       panel_markup_2,
                                       panel_markup_3,
                                       stream_markup,
                                       telegram_markup)
...
@mausic.on_callback_query(filters.regex("PanelMarkup") & ~BANNED_USERS)
@languageCB
...
@mausic.on_callback_query(filters.regex("MainMarkup") & ~BANNED_USERS)
@languageCB
...
@mausic.on_callback_query(filters.regex("Pages") & ~BANNED_USERS)
@languageCB
async def del_back_playlist(client, CallbackQuery, _):
    await CallbackQuery.answer()
    callback_data = CallbackQuery.data.strip()
...
@mausic.on_callback_query(filters.regex("ADMIN") & ~BANNED_USERS)
@languageCB
async def del_back_playlist(client, CallbackQuery, _):
    callback_data = CallbackQuery.data.strip()
    callback_request = callback_data.split(None, 1)[1]
