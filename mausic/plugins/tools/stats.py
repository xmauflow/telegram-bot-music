...
from mausic import YouTube, mausic
from mausic.core.userbot import assistants
from mausic.misc import SUDOERS, pymongodb
from mausic.plugins import ALL_MODULES
from mausic.utils.database import (get_global_tops,
                                    get_particulars, get_queries,
                                    get_served_chats,
                                    get_served_users, get_sudoers,
                                    get_top_chats, get_topp_users)
from mausic.utils.decorators.language import language, languageCB
from mausic.utils.inline.stats import (back_stats_buttons,
                                        back_stats_markup,
                                        get_stats_markup,
...
@mausic.on_callback_query(filters.regex("GetStatsNow") & ~BANNED_USERS)
@languageCB
async def top_users_ten(client, CallbackQuery: CallbackQuery, _):
    ...
    if what in ["Users", "Chats"]:
        for items, count in list_arranged.items():
            ...
            try:
                extract = (
                    (await mausic.get_users(items)).first_name
                    if what == "Users"
                    else (await mausic.get_chat(items)).title
                )
                if extract is None:
                    continue
...
@mausic.on_callback_query(filters.regex("TopOverall") & ~BANNED_USERS)
@languageCB
...
@mausic.on_callback_query(filters.regex("bot_stats_sudo"))
@languageCB
async def overall_stats(client, CallbackQuery, _):
...
@mausic.on_message(filters.command("stats", [".", "^", "-", "!"]) & SUDOERS)
async def statsmausic(_, message: Message):
    cht = len(await get_served_chats())
    usr = len(await get_served_users())
    await mausic.send_message(message.chat.id, 
        f"📊 **Current Stats:**\n\n• **Users:** `{usr}`\n• **Groups:** `{cht}`")
